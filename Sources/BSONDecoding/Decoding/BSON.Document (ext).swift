...
extension BSON.Document {
    @inlinable var items: Iterator { .init(input: .init(self.bytes)) }

    @inlinable public func parsed<Key>(_: Key.Type = Key.self) -> BSON.KeyspaceDecoder<Key> {
        .init(input: self.items)
    }

    @inlinable package func parseAll() throws -> [BSON.FieldDecoder<BSON.Key>] {
