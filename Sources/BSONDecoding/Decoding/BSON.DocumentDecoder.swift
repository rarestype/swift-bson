...
extension BSON {
    ...
    @frozen public struct DocumentDecoder<CodingKey>
        where   CodingKey: RawRepresentable<String>,
        CodingKey: Hashable,
        CodingKey: Sendable {
        @usableFromInline var index: [CodingKey: BSON.AnyValue]

        @inlinable public init(_ index: [CodingKey: BSON.AnyValue] = [:]) {
            self.index = index
        }
    }
}
extension BSON.DocumentDecoder: BSON.Decoder {
    ...
    @inlinable public init(parsing bson: borrowing BSON.AnyValue) throws {
...
extension BSON.DocumentDecoder {
    ...
    @inlinable public init(parsing bson: borrowing BSON.Document) throws {
        try self.init(bson: bson.items)
    }

    @inlinable init(bson: consuming BSON.Document.Iterator) throws {
        self.init()
        while let (key, value): (String, BSON.AnyValue) = try bson.next() {
            guard
            let key: CodingKey = .init(rawValue: key) else {
...
extension BSON.DocumentDecoder {
    @inlinable public func contains(_ key: CodingKey) -> Bool {
    ...
    @inlinable public var indexedFields: [CodingKey: BSON.AnyValue] { self.index }

    @inlinable public var single: BSON.FieldDecoder<CodingKey> {
    ...
    @inlinable public subscript(key: CodingKey) -> BSON.FieldAccessor<CodingKey> {
        .init(key: key, value: self.index[key])
    }
    @inlinable public subscript(key: CodingKey) -> BSON.FieldDecoder<CodingKey>? {
