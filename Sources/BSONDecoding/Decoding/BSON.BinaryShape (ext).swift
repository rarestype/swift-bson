extension BSON.BinaryShape {
    ...
    @inlinable public func expect(size bytes: Int) throws {
        guard self.bytes == bytes else {
    ...
    @inlinable public func expect(that predicate: (_ bytes: Int) throws -> Bool) throws {
        guard try predicate(self.bytes) else {
