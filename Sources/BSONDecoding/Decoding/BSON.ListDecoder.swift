...
extension BSON.ListDecoder {
    ...
    @inlinable public subscript(_: (BSON.EndIndex) -> ()) -> BSON.FieldAccessor<Int> {
        mutating get throws {
            guard
            let value: BSON.AnyValue = try self.input.next() else {
    ...
    @inlinable public subscript(_: (BSON.EndIndex) -> ()) -> BSON.FieldDecoder<Int>? {
        mutating get throws {
            guard
            let value: BSON.AnyValue = try self.input.next() else {
                return nil
