extension BSON {
    ...
    @frozen public struct FieldAccessor<Key> where Key: Sendable {
        public let key: Key
        public let value: BSON.AnyValue?

        @inlinable public init(key: Key, value: BSON.AnyValue?) {
            self.key = key
            self.value = value
        }
    }
...
extension BSON.FieldAccessor {
    ...
    @inlinable public func decode() throws -> BSON.AnyValue {
...
extension BSON.FieldAccessor: BSON.TracingDecoder {
    ...
    @inlinable public func decode<T>(with decode: (BSON.AnyValue) throws -> T) throws -> T {
