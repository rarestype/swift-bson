extension BSON {
    ...
    public protocol TracingDecoder {
...
extension BSON.TracingDecoder {
    @inlinable public func decode<CodingKey, T>(
        ...
        with decode: (BSON.DocumentDecoder<CodingKey>) throws -> T
    ) throws -> T {
        try self.decode { try decode(try .init(parsing: $0)) }
    }
    @inlinable public func decode<T>(
        with decode: (BSON.DocumentDecoder<BSON.Key>) throws -> T
    ) throws -> T {
        try self.decode { try decode(try .init(parsing: $0)) }
    }
    @inlinable public func decode<T>(
        with decode: (inout BSON.ListDecoder) throws -> T
    ) throws -> T {
        try self.decode {
