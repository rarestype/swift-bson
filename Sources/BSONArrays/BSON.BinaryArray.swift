...
extension BSON {
    @frozen public struct BinaryArray<Element>: Sendable where Element: BinaryPackable {
        @usableFromInline var buffer: BSON.BinaryBuffer<Element.Storage>

        @inlinable init(buffer: BSON.BinaryBuffer<Element.Storage>) {
            self.buffer = buffer
        }
    }
}
...
extension BSON.BinaryArray: BSONBinaryDecodable {
    public init(bson: BSON.BinaryDecoder) throws {
