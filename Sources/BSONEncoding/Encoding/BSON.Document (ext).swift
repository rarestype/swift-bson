...
extension BSON.Document {
    ...
    @inlinable public init(encoding encodable: __shared some BSONDocumentEncodable) {
        self.init(with: encodable.encode(to:))
    }

    @inlinable public init<CodingKey>(
        _: CodingKey.Type = CodingKey.self,
        with encode: (inout BSON.DocumentEncoder<CodingKey>) throws -> ()
    ) rethrows {
        self.init()
