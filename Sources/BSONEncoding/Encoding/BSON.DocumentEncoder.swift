extension BSON {
    @frozen public struct DocumentEncoder<CodingKey> where CodingKey: RawRepresentable<String> {
        @usableFromInline var output: BSON.Output

        @inlinable public init(_ output: BSON.Output) {
            self.output = output
        }
    }
}
...
extension BSON.DocumentEncoder {
    ...
    @inlinable public subscript(key: CodingKey) -> BSON.FieldEncoder {
    ...
    @inlinable public subscript(with key: some RawRepresentable<String>) -> BSON.FieldEncoder {
        _read   { yield  self.output[with: .init(key)] }
        _modify { yield &self.output[with: .init(key)] }
    }
}
extension BSON.DocumentEncoder {
    ...
    @inlinable public subscript<Value>(key: CodingKey) -> Value? where Value: BSONEncodable {
        get { nil }
