...
extension BSON.Input {
    ...
    @discardableResult
    @inlinable public mutating func parse(through byte: UInt8) throws -> Range<Int> {
    ...
    @inlinable public mutating func parse(as _: String.Type = String.self) throws -> String {
    ...
    @inlinable public mutating func parse(
        as _: BSON.Identifier.Type = BSON.Identifier.self
    ) throws -> BSON.Identifier {
        let start: Int = self.index
        if  let end: Int = self.bytes.index(
                self.index, offsetBy: 12,
    ...
    /// Parses a boolean.
    @inlinable public mutating func parse(as _: Bool.Type = Bool.self) throws -> Bool {
        switch self.next() {
        case 0?:
            return false
        case 1?:
    ...
    @inlinable public mutating func parse(
        as _: BSON.Decimal128.Type = BSON.Decimal128.self
    ) throws -> BSON.Decimal128 {
        let low: UInt64 = try self.parse(as: UInt64.self)
        let high: UInt64 = try self.parse(as: UInt64.self)
        return .init(high: high, low: low)
    }
    @inlinable public mutating func parse(
        as _: UnixMillisecond.Type = UnixMillisecond.self
    ) throws -> UnixMillisecond {
        .init(index: try self.parse(as: Int64.self))
    }
    @inlinable public mutating func parse(
        as _: BSON.Regex.Type = BSON.Regex.self
    ) throws -> BSON.Regex {
    ...
    @inlinable public mutating func parse<LittleEndian>(
        as _: LittleEndian.Type = LittleEndian.self
    ) throws -> LittleEndian
    ...
    @inlinable public mutating func parse<Frame>(_: Frame.Type) throws -> ArraySlice<UInt8>
    ...
    @inlinable public mutating func parse<View>(as _: View.Type = View.self) throws -> View
    ...
    /// `index`.
    @inlinable public var remaining: ArraySlice<UInt8> {
        self.bytes.suffix(from: self.index)
    }

    /// Asserts that there is no input remaining.
    @inlinable public func finish() throws {
    ...
    @inlinable public func expected(
        _ expectation: BSON.InputError.Expectation
    ) -> BSON.InputError {
        .init(
...
extension BSON.Input {
    ...
    @inlinable public mutating func parse(variant: BSON.AnyType) throws -> BSON.AnyValue {
        switch variant {
        case .double:
