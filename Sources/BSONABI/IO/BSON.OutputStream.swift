...
extension BSON.OutputStream {
    ...
    @inlinable public mutating func serialize(cString: String) {
        self.append(cString.utf8)
        self.append(0x00)
    }
    /// Serializes a fixed-width integer in little-endian byte order.
    @inlinable public mutating func serialize(integer: some FixedWidthInteger) {
        withUnsafeBytes(of: integer.littleEndian) {
            self.append($0)
        }
    }

    @inlinable public mutating func serialize(
        utf8: BSON.UTF8View<some BidirectionalCollection<UInt8>>
    ) {
    ...
    @inlinable public mutating func serialize(document: BSON.Document) {
        self.serialize(integer: document.header)
        self.append(document.bytes)
        self.append(0x00)
    }
    @inlinable public mutating func serialize(list: BSON.List) {
