extension BSON {
    ...
    @frozen public struct BinaryBuffer<Element> where Element: BitwiseCopyable {
        @usableFromInline package var bytes: ArraySlice<UInt8>
        public let count: Int

        @inlinable init(bytes: ArraySlice<UInt8>, count: Int) {
            self.bytes = bytes
            self.count = count
        }
    }
...
extension BSON.BinaryBuffer {
    ...
    /// buffer length.
    @inlinable public init(bytes: ArraySlice<UInt8>) throws {
...
extension BSON.BinaryBuffer: ExpressibleByArrayLiteral {
    ...
    @inlinable public init(arrayLiteral: Never...) { self.init(count: 0) }
}
extension BSON.BinaryBuffer: RandomAccessCollection {
    @inlinable public var startIndex: Int { 0 }

    @inlinable public var endIndex: Int { self.count }
