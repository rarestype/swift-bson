extension BSON {
    ...
    @frozen public struct BinaryShape: Hashable, Sendable {
        public let bytes: Int

        @inlinable public init(bytes: Int) {
            self.bytes = bytes
        }
    }
}
extension BSON.BinaryShape {
    ...
    @inlinable public func expect(multipleOf stride: Int) throws -> Int {
        if  self.bytes == 0 {
            return 0
        }

        guard stride > 0,
        case (let count, remainder: 0) = self.bytes.quotientAndRemainder(
            dividingBy: stride
        ) else {
