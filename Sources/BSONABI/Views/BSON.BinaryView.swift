extension BSON {
    /// A BSON binary array.
    @frozen public struct BinaryView<Bytes> where Bytes: RandomAccessCollection<UInt8> {
        ...
        public let subtype: BinarySubtype

        @inlinable public init(subtype: BinarySubtype, bytes: Bytes) {
...
extension BSON.BinaryView: Equatable {
    ...
    @inlinable public static func == (
        a: Self,
        b: BSON.BinaryView<some RandomAccessCollection<UInt8>>
    ) -> Bool {
        a.subtype == b.subtype &&
...
extension BSON.BinaryView<ArraySlice<UInt8>>: BSON.BufferTraversable {
    public typealias Frame = BSON.BinaryFrame
    ...
    ///
    /// >   Complexity: O(1)
    @inlinable public init(slicing bytes: ArraySlice<UInt8>) throws {
        guard let code: UInt8 = bytes.first else {
            throw BSON.BinaryViewError.init(expected: .subtype)
        }
        guard let subtype: BSON.BinarySubtype = .init(rawValue: code) else {
        ...
        else if let start: Int = bytes.index(
                start, offsetBy: 4,
