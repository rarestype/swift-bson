extension BSON {
    ...
    @frozen public struct Document: Sendable {
...
extension BSON.Document: BSON.BufferTraversable {
    public typealias Frame = BSON.DocumentFrame

    /// Stores the argument in ``bytes`` unchanged. Equivalent to ``init(bytes:)``.
    ///
    /// >   Complexity: O(1)
    @inlinable public init(slicing bytes: ArraySlice<UInt8>) {
    ...
    @inlinable public var bytes: ArraySlice<UInt8> { self.output.bytes }
}
extension BSON.Document {
    /// Indicates if this document contains no fields.
    @inlinable public var isEmpty: Bool { self.bytes.isEmpty }

    /// The length that would be encoded in this document’s prefixed header.
    /// Equal to ``size``.
    @inlinable public var header: Int32 { .init(self.size) }
    ...
    @inlinable public var size: Int { 5 + self.bytes.count }
    ...
    @inlinable public init(list: BSON.List) {
        self.init(bytes: list.bytes)
    }

    @inlinable public mutating func append(contentsOf other: Self) {
...
extension BSON.Document {
    ...
    @inlinable public init(fields: some Collection<(key: BSON.Key, value: BSON.AnyValue)>) {
    ...
    @inlinable public init(key: BSON.Key, value: BSON.AnyValue) {
        self.init(
...
extension BSON.Document {
    ...
    @inlinable public init(_ bson: Self) {
        self = bson
    }

    @available(*, deprecated, message: "BSON.Document is already a BSON.Document")
    @inlinable public init(bson: Self) {
        self = bson
    }
}
