...
extension BSON.KeyedDecoder {
    ...
    func diagnose<T>(
        ...
        _ decode: (BSON.AnyValue) throws -> T?
    ) throws -> T {
        var path: [any CodingKey] {
            self.codingPath + CollectionOfOne<any CodingKey>.init(key)
        }
        guard let value: BSON.AnyValue = self.items[.init(key)] else {
        ...
        do {
            ...
            throw DecodingError.init(
                annotating: BSON.TypecastError<T>.init(
                    invalid: value.type
                ),
                initializing: T.self,
                path: path
...
extension BSON.KeyedDecoder: KeyedDecodingContainerProtocol {
    ...
    public func singleValueContainer<T>(
        forKey key: T,
        typed _: T.Type = T.self
    ) throws -> BSON.SingleValueDecoder
        where T: CodingKey {
        let value: BSON.AnyValue = try self.diagnose(key){ $0 }
        let decoder: BSON.SingleValueDecoder = .init(
            value,
